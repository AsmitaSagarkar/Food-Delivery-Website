      ...
      <form class="text-gray-400 bg-gray-900 body-font" onSubmit={getSearch}>
        <div class="container py-10 mx-auto">
          ...
          <div class="flex lg:w-1/2 w-full sm:flex-row flex-col mx-auto px-8 sm:px-0 items-end sm:space-x-4 sm:space-y-0 space-y-4">
            <div class="relative sm:mb-0 flex-grow w-full mr-11">
              <label for="full-name" class="leading-7 text-sm text-gray-400">Enter Ingredient (separate them using comma.)</label>
              <input type="text" id="full-name" name="full-name" value={search} onChange={updateSearch} class="w-full bg-gray-800 bg-opacity-40 rounded border border-gray-700 focus:border-pink-tone focus:pink-tone focus:bg-transparent text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
            </div>

            <button type="submit" class="text-black bg-pink-tone border-0 py-2 px-8 focus:outline-none  rounded text-lg">Search</button>
          </div>
        </div>
      </form>
